...
import static socialledger.model.MoneyHelper.ZERO_RANDS;
...
public class Expense {
    ...
    public Payment payPaymentRequest(UUID paymentRequestId, Person personWhoShouldPayBack, LocalDate date) {
        return this.listOfPaymentRequests().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new SocialLedgerException("Cannot find payment request"))
    ...
    private void checkDate(LocalDate date) {
        if (date.isAfter(LocalDate.now())) throw new SocialLedgerException("Expense cannot be in the future");
    }
